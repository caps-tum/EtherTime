...
vendor, which we use later as a comparison for different environment
configurations.
We use the default PTP/NTP profile wherever applicable.
per run).
We collect data in 20-minute runs (which corresponds to just above 1000 samples per run).
...
A higher frequency does not significantly improve clock synchronization,
e.g.,~LinuxPTP on the Raspberry-Pi 4 reported the best synchronization quality
at a sampling rate of 2\,Hz or lower.
On the other hand, a higher frequency negatively impacts the stability of the
