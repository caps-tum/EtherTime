...
which in turn consists of around \numSamplesPerRunApprox{} samples.
%Despite decent measurement lengths and number of samples,
However, there is a significant amount of noise during each run,
\todo{Find a good metric to use here, perhaps std-dev + chebyshev law to deduce some intervals}
and sometimes also discrepancies between runs.
%which can lead to completely different results for identical measurement runs
...
\assert{\cmpMaxArg}{ptpd}
%In absolute terms, across independent measurements,
...
latency to jump from \fTimeKey{ptpd/min} up to \fTimeKey{ptpd/max}, which
corresponds to an increase of
\fRelative[-1]{\ptpKey{ptpd/max}/\ptpKey{ptpd/min}} not only momentarily, but
...
of \fTimeKey{linuxptp/min} and \fTimeKey{linuxptp/max} between its best and worst runs,
corresponding to just \fRelative{\ptpKey{linuxptp/max}/\ptpKey{linuxptp/min}} difference.
...
\fTime[1]{\ptpKey{chrony/max}-\ptpKey{chrony/min}}.
...
\fpeval{round(\baselineTotalMinutesRuntime/60)} hours of runtime and
\fpeval{round(\baselineTotalMinutesRuntime*60)} samples collected for just one
type of benchmark).
