...
\section{Time Synchronization Overview}
...
%which need to be perpetually fulfilled by any timing system.
\begin{table}
    \centering
    \caption{Properties of Timekeeping Systems}
    %Invariants (I1-I4) and Challenges (C1 and C2)}
    \begin{tabular}{l}
    \toprule
...
However, for successful time synchronization, two prerequisites should hold,
...
%However, time synchronization algorithms must first address two principle
%challenges, % to guarantee invariant I4.
%C1 and C2, which are also listed in \cref{tab:invariants}.
%C1 is especially difficult across packet-switched networks with best-effort
%quality of service due to the inherent packet-delay variation present.
%Even if C1 is tackled, modifying current time has repercussions
%(as discussed above),
%so most time synchronization programs have rules limiting under what conditions
%each invariant I1-I3 may be violated.
...
Throughout this measurement study, we will determine what can be expected of time
...
%\textbf{PTP overview.}
%%In the following,
%We provide a brief overview of one of the well known time
%synchronization protocol, PTP~\cite{ptp-spec},
%which is also the primary focus of this study.
%%which typically tries
%%to minimize disruptions by breaking the lowest priority invariant possible to
%%keep clocks in sync, exploiting the fact that I3 is more of a soft requirement.
...
\begin{figure*}
    ...
    \begin{tikzpicture}[
        ...
        \node[node distance=1cm, stage] (bmca) {Best Master Clock Algorithm};
    ...
    (e.g., violation of invariants I2 and potentially I1).
...
While S6, the \textit{stable} stage, is usually of most interest for evaluation,
...
%We study both types of platforms, with and without hardware support.
