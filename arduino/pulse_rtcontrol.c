...
/******************** GPIO read/write *******************/
#define GPIO_PIN_NUMBER 24
#define GPIO_PIN_HI *gpset = (1 << GPIO_PIN_NUMBER)  // GPIO 2 output hi
#define GPIO_PIN_LO *gpclr = (1 << GPIO_PIN_NUMBER)  // GPIO 2 output lo
...
set GPIO 2 level
  GPIO_PIN_HI;    // GPIO 2 hi
  GPIO_PIN_LO;    // GPIO 2 lo
...
int measureints()
  ...
  do
    {
    timend = *timer + 1;  // (old 10) now (1) us delay
    TIMERLOOP;
    if( (*timer - timend) != 0 )
...
void hardwaretest()
  ...
  int n;
                 ...
                 // set GPIO 2 as output
  INP_GPIO(GPIO_PIN_NUMBER);   // must do this first to zero control bits
  OUT_GPIO(GPIO_PIN_NUMBER);   // set as output
  ...
  for(n = 0 ;  n < 15  ; ++n)
    {
    timend += 1000 * 1000;    // 10us
    TIMERLOOP;

    GPIO_PIN_HI;          // output GPIO 2 hi

    timend += 1000 * 1000;    // 10us
    TIMERLOOP;

    GPIO_PIN_LO;         // output GPIO 2 lo
    }
